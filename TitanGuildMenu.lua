...
function TitanPanelRightClickMenu_PrepareGuildMenu()
	...
	if ( _G["UIDROPDOWNMENU_MENU_LEVEL"] == 2 ) then
		if (_G["UIDROPDOWNMENU_MENU_VALUE"]) then
			-- chat
			if (_G["UIDROPDOWNMENU_MENU_VALUE"] == TITAN_GUILD_MENU_CHAT_TEXT) then
			...
			-- sort
			elseif (_G["UIDROPDOWNMENU_MENU_VALUE"] == TITAN_GUILD_MENU_SORT_TEXT) then
				-- sort options
				TitanPanelRightClickMenu_AddTitle(_G["UIDROPDOWNMENU_MENU_VALUE"], _G["UIDROPDOWNMENU_MENU_LEVEL"]);
				local choiceIndex;
			...
			elseif (_G["UIDROPDOWNMENU_MENU_VALUE"] == TITAN_GUILD_MENU_TOOLTIP_TEXT) then
				-- tooltip options
				TitanPanelRightClickMenu_AddTitle(_G["UIDROPDOWNMENU_MENU_VALUE"], _G["UIDROPDOWNMENU_MENU_LEVEL"]);					
				local choiceIndex;
				for choiceIndex = 1, table.getn(sortChoicesValues) do
				...
				UIDropDownMenu_AddButton(info, _G["UIDROPDOWNMENU_MENU_LEVEL"]);
				TitanPanelRightClickMenu_AddCommand(TITAN_GUILD_MENU_HIDE, TITAN_GUILD_ID, TITAN_GUILD_BUTTON_CLOSEMENU, _G["UIDROPDOWNMENU_MENU_LEVEL"]);
			elseif (_G["UIDROPDOWNMENU_MENU_VALUE"] == TITAN_GUILD_MENU_FILTER_TEXT) then
				-- filter options
				TitanPanelRightClickMenu_AddTitle(_G["UIDROPDOWNMENU_MENU_VALUE"], _G["UIDROPDOWNMENU_MENU_LEVEL"]);
				-- lvl
				info = {};
				...
				UIDropDownMenu_AddButton(info, _G["UIDROPDOWNMENU_MENU_LEVEL"]);												
				TitanPanelRightClickMenu_AddCommand(TITAN_GUILD_MENU_HIDE, TITAN_GUILD_ID, TITAN_GUILD_BUTTON_CLOSEMENU, _G["UIDROPDOWNMENU_MENU_LEVEL"]);					
			elseif (_G["UIDROPDOWNMENU_MENU_VALUE"] == TITAN_GUILD_MENU_CONFIGURE_UPDATE_TIME) then
				-- update options
				TitanPanelRightClickMenu_AddTitle(_G["UIDROPDOWNMENU_MENU_VALUE"], _G["UIDROPDOWNMENU_MENU_LEVEL"]);
				local choiceIndex;
			...
			-- player submenus
				...
				if (TitanGetVar(TITAN_GUILD_ID, "ShowAdvancedMenus")) then
					...
					if (priorAdvMenuValue ~= _G["UIDROPDOWNMENU_MENU_VALUE"]) then
						TitanPanelGuildButton_InitPaging();
						priorAdvMenuValue = _G["UIDROPDOWNMENU_MENU_VALUE"];
					end
					-- generate lvl 2 player lists based on rank
					TitanPanelRightClickMenu_AddTitle(GuildControlGetRankName(_G["UIDROPDOWNMENU_MENU_VALUE"]), _G["UIDROPDOWNMENU_MENU_LEVEL"]);
					TitanPanelGuildButton_ComputeAdvancedPages(table.getn(masterTable[_G["UIDROPDOWNMENU_MENU_VALUE"]].members));
					TitanPanelGuildButton_BuildBackwardPageControl();
					for numMember = currIndex, maxIndex do
						if (masterTable[_G["UIDROPDOWNMENU_MENU_VALUE"]].members[numMember]) then
							info = {};
							info.text = masterTable[_G["UIDROPDOWNMENU_MENU_VALUE"]].members[numMember];
							info.hasArrow = 1;
							UIDropDownMenu_AddButton(info, _G["UIDROPDOWNMENU_MENU_LEVEL"]);
						end				
					end
		...
		return
		-- End Level 2 -------------------------------------
	elseif (_G["UIDROPDOWNMENU_MENU_LEVEL"] == 3) then
		-- Level 3 ------------------------------------------
		if (_G["UIDROPDOWNMENU_MENU_VALUE"] == TITAN_GUILD_MENU_FILTER_CLASS) then
			TitanPanelGuildButton_BuildClassFilterMenu();
		else
			if (_G["UIDROPDOWNMENU_MENU_VALUE"] == TITAN_GUILD_MENU_TOOLTIPSIZE) then
				local start = 5
...
function TitanPanelGuildButton_BuildInteractionMenu()
	...
	info.value = _G["UIDROPDOWNMENU_MENU_VALUE"];
	info.func = function() TitanPanelGuildButton_GuildWhisper(_G["UIDROPDOWNMENU_MENU_VALUE"]) end;
	UIDropDownMenu_AddButton(info, _G["UIDROPDOWNMENU_MENU_LEVEL"]);			
	-- invite
	info = {};
	info.text = TITAN_GUILD_MENU_ADVANCED_INVITE_TEXT;
	info.value = _G["UIDROPDOWNMENU_MENU_VALUE"];
	info.func = function() TitanPanelGuildButton_InviteToGroup(_G["UIDROPDOWNMENU_MENU_VALUE"]) end;
	UIDropDownMenu_AddButton(info, _G["UIDROPDOWNMENU_MENU_LEVEL"]);
	-- who
	info = {};
	info.text = TITAN_GUILD_MENU_ADVANCED_WHO_TEXT;
	info.value = _G["UIDROPDOWNMENU_MENU_VALUE"];
	info.func = function() TitanPanelGuildButton_SendWhoRequest(_G["UIDROPDOWNMENU_MENU_VALUE"]) end;
	UIDropDownMenu_AddButton(info, _G["UIDROPDOWNMENU_MENU_LEVEL"]);
	-- friend
	info = {};
	info.text = TITAN_GUILD_MENU_ADVANCED_FRIEND_TEXT;
	info.value = _G["UIDROPDOWNMENU_MENU_VALUE"];
	info.func = function() TitanPanelGuildButton_AddFriend(_G["UIDROPDOWNMENU_MENU_VALUE"]) end;
	UIDropDownMenu_AddButton(info, _G["UIDROPDOWNMENU_MENU_LEVEL"]);	
end

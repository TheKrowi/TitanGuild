...
function TitanPanelGuildButton_ComputePages()
	...
	pagingRemainder = (math.modf(counterToUse,TITAN_GUILD_LIST_THRESHOLD));
	-- compute pages
...
function TitanPanelGuildButton_ComputeAdvancedPages(numInRank)
	...
	-- determine the number of pages are required to render online members
	pagingRemainder = (math.modf(numInRank,TITAN_GUILD_LIST_THRESHOLD));
	-- compute pages
	if (pagingRemainder ~= 0) then
...
function TitanPanelGuildButton_BuildForwardPageControl()
	...
	if (numPages > currPage) then
		...
		if (numPages ~= currPage) then
			...
			TitanPanelRightClickMenu_AddCommand(forwardText, TITAN_GUILD_ID, TITAN_GUILD_BUTTON_FORWARDPAGE, _G["L_UIDROPDOWNMENU_MENU_LEVEL"]);
		end
	end
end
...
function TitanPanelGuildButton_BuildBackwardPageControl()
	...
	if (currPage ~= 1) then
		...
		TitanPanelRightClickMenu_AddCommand(backwardText, TITAN_GUILD_ID, TITAN_GUILD_BUTTON_BACKWARDPAGE, _G["L_UIDROPDOWNMENU_MENU_LEVEL"]);
	end
end
